...
const CN = `https://joshweb.click/new/gpt-3_5-turbo?prompt=`
...
module.exports.handleReply = async function({
    ...
    const res = await get(`https://joshweb.click/new/gpt-3_5-turbo?prompt=${encodeURI(event.body)}`);
   if (res.data.error) return api.sendMessage(`${res.data.error}`, event.threadID, (err, data) => global.client.hhandleReply.push({
       name: this.config.name,
       messageID: data.messageID,
       ask: event.body
   }), event.messageID); else api.sendMessage({body: `${res.data.result.reply}`
},event.threadID, (err, data) => global.client.handleReply.push({
        name: this.config.name, messageID: data.messageID
    }), event.messageID);
};
