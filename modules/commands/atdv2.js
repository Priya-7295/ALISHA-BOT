  ...
  try {
    ...
    if (isURL(str)) {
      if (/fb|facebook/.test(str)) {
        ...
        const res = await axios.get(`https://joshweb.click/facebook?url=${str}`);
        send({
          body: `${head('FaceBook')}`, attachment: await streamURL(res.data.result, 'mp4')
        });
      }
   ...
   /*   else if(/catbox\.moe/.test(str)){
      ...
      else if (/xvos\.com/.test(str)) {
      const res = (await axios.get(`https://joshweb.click/prn/download?url=${str}`)).data
      let attachment = [];
                      if (res.data.result != null) {
      attachment = await streamURL(res.data.contentUrl.video_url,"mp4")
      } else {
      attachment = await streamURL(res.data.result.contentUrl.Default_Quality[0], 'jpg');
                      }
      send({body: `${head("XVIDEOS AUTODOWN")}\n→ Tiêu đề: ${res.data.result.name}`,attachment})
      }
      /* TỰ ĐỘNG TẢI ẢNH HOẶC VIDEO PINTEREST */ 
      else if (/(^https:\/\/)((www)\.)?(xvideos|pin)*\.(com|it)\//.test(str)) {
                const res = await axios.get(`https://joshweb.click/prn/download?url=${str}`);
                send({
                    body: `${head('AUTODOWN XVIDEOS')}\n- tiêu đề: ${res.data.result.name}`, attachment: await streamURL(res.data.result.contentUrl.Default_Quality, 'mp4')});
            } 
      /* TỰ ĐỘNG TẢI ẢNH HOẶC VIDEO INSTAGRAM */ 
      else if (/instagram\.com/.test(str)) {
                const res = await axios.get(`https://joshweb.click/api/instadl?url=${str}`);
                const {
                    result = [{}],
                    result
                } = res.data;
                ...
                if (videos[0] != undefined) {
                    attachment = await streamURL(result[0], 'mp4');
                ...
                send({
                    body: `${head('INSTAGRAM')}`, attachment
                });
            }
        }
